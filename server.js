...
app.use(
  "/robots.txt",
  express.static(
    path.join(process.cwd(), "./reagent-frontend/public/robots.txt")
  )
);
app.use(
  "/js",
  express.static(path.join(process.cwd(), "./reagent-frontend/public/js"))
);
app.use(
  "/css",
  express.static(path.join(process.cwd(), "./reagent-frontend/public/css"))
);
app.use(
  "/music",
  express.static(path.join(process.cwd(), "./reagent-frontend/public/music"))
);
app.use(
  "/blog",
  express.static(path.join(process.cwd(), "./reagent-frontend/public/blog"))
);
app.use(
  "/downloads",
  express.static(
    path.join(process.cwd(), "./reagent-frontend/public/downloads")
  )
);
app.use(
  "/images",
  express.static(path.join(process.cwd(), "./reagent-frontend/public/images"))
);
app.use(
  "/data",
  express.static(path.join(process.cwd(), "./reagent-frontend/public/data"))
);
const postersPath = "./reagent-frontend/public/images/presentaciones";
app.use("/posters", express.static(path.join(process.cwd(), postersPath)));
...
const seoBase = {
  title: "| Echoic Space",
  description:
    "Música experimental de Diego Villaseñor. Grabaciones, partituras, código y análisis.",
  image: {
...
const makeSinglePageOpenGraph = (parentSlug, singlePageSlug, db, req, res) => {
  const page = R.pipe(
    R.find((m) => R.path(["attributes", "slug"], m) === singlePageSlug),
    R.pathOr({}, ["attributes"]),
    log
  )(db);
...
app.get("/music", (req, res) => {
  let openGraph = {
    ...
    title: "| Música",
    description:
      "Archivo de composiciónes, improvisaciónes y live coding. Audios, partituras, código y análisis",
    image: {
  ...
  axios
    .get(
      `https://freesound.org/apiv2/search/text?token=${token}&query=field+recordings+${query.replace(
        " ",
        "+"
      )}&fields=username,tags,previews,url`
    )
    .then((r) => r.data)
...
app.get("/get-posters", (req, res) => {
  fs.readdir(postersPath, (err, imageNames) =>
    res.json(imageNames.map((img) => "/posters/" + img))
  );
});
...
app.listen(process.env.PORT || 3000, () =>
  console.log("echoic-space running on port " + process.env.PORT + "!")
);
